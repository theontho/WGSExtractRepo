...
class ReferenceLibrary:
    ...
    #  https://hgdownload.soe.ucsc.edu/goldenPath/hg38/liftOver/ should be added to the reference library
    #  (especially new T2T ones) as should VCF WGS annotations, BED files, etc
    ...
    def refgen_liftover(ref_genome):
        ...
        return  ("hg37",    93) if ref_genome in ["hg38"] else \
                ("hs37d5",  86) if ref_genome in ["hs38", "hs38a", "hs38d1", "hs38d1a", "hs38d1s", "hs38DH",
                                                  "hs38d1v"] else \
                ...
                ("T2Tv20",  25) if ref_genome in ["T2Tv11", "T2Tv10", "T2Tv09", "THGySeqp", "THGv27", "THGv20",
                                                  "THG1243v3", "HPPv11", "HPPv1", "T2Tv20a"] else \
                ("error", 0)    # hg37w, T2Tv20a

    def get_reference_vcf_qFN(self, build, sqname, ttype="FullGenome"):
        if ttype == "Yonly":  # Y-only Variants from yBrowse DB; updated daily.  Thomas Krahn suggest liftover to Build38
        ...
        elif ttype == "Microarray":  # Special list of CombinedKit Variants (union of all generated kits)
        ...
        else:  # ttype == "FullGenome" (use dbSNP)      # Found in reference_library; from https://ftp.ncbi.nlm.nih.gov/snp/latest_release/VCF/
            pass  # TBD not yet implemented nor called; these files are very large. 15GB each or larger. should only be used for annotation
    ...
    def ask_reference_genome(self, inBAM=True):
        ...
        Also used when need to ask for a reference genome during  alignment or cannot find a liftover match for realign.
        ...
        textToDisplay = ""
        if inBAM:  # Failed trying to determine Reference Genome of BAM; so ask user (setting initial)
            ...
            textToDisplay += wgse.lang.i18n['PleaseSelectReferenceGenomeCur']
        else:
            textToDisplay += wgse.lang.i18n['PleaseSelectReferenceGenomeNew']
        ...
        if inBAM:       # Only display if in BAM (cannot select Unknown if in Align command)
...
'''
    ...
    # for bam in pybam.read(r"/WGS/RandyYseq/22731_bwa-mem_hg19_sorted.bam"):
