...
def _button_CombinedKit(parent_window, parallel=False):
    ...
    if os.path.exists(CombinedKitZIP_oFN) and \
       ...
       os.path.getsize(CombinedKitZIP_oFN) > minCbnKitSize:
        ...
        query_pattern = r"'%ID\t%CHROM\t%POS[\t%TGT]\n'"
        reorder_pattern = (r"'s/chr//; s/\tM\t/\tMT\t/g; s/\///; s/\.\.$/--/; "
                           r"s/TA$/AT/; s/TC$/CT/; s/TG$/GT/; s/GA$/AG/; s/GC$/CG/; s/CA$/AC/'")
        tab_pattern = r"$'\t'"
        ...
        commands += (
            ...
            f'{bcftools} query -f {query_pattern} {temp_annotated_vcf} -o {temp_result_tab}\n'
            f'{sed} {reorder_pattern} {temp_result_tab} | {sort} -t {tab_pattern} -k2,3 -V > {temp_sorted_result_tab}\n'
