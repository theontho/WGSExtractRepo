...
Converted from the [original Google Doc version](https://docs.google.com/document/d/1HBj317OMeq26EmpwVWlAuzZsr2bfWh8Y58A8wAYWVoc) with a "download as markdown" option.   Putting this in the repo can help AI agents with context for changes.   Open the original google doc, download as markdown and run `scripts/extract_markdown_images.py` to extract the images.

## Alpha v4 Edition 15 July 2022 release Combined Reference and User Manual

[*Installation and Starting*](#7-installing-and-starting-wgs-extract) *section at the end.*
...
This **WGSE** v4 major release contains many underlying improvements.  See the [release notes](#8.3-release-notes) for more detail. The biggest new benefits are support for the new T2T reference model library, FASTQ file support, and VCF file generation. Other underlying improvements are made as well.
...
See the [Installing and Starting Section](#heading) moved to the end of the document if you have not yet installed the **WGSE** tool. We now delve directly into how to use the tool once you click the script to start it.

# 1 Running the WGS Extract Tool

There are actually four main tools delivered: the WGSExtract program itself, the (Reference Genome) Library tool, the installer, and the uninstaller. The focus here is on the main WGS Extract tool.  For more information on the [Reference Library subsystem and its tool](#6-reference-genomes), see the later chapter covering it more. And  the [Installing and Starting Section](#heading) for more on the installer and uninstaller.
...
* Dante Labs WGS/WGZ done on an Illumina NovaSeq 6000 sequencer and delivered mapped to the hs37d5 reference model,   
* Nebula Genomics WGS done in the BGI Labs on a MGI DNB sequencer and delivered mapped to the hs38xx reference model, and   
* ySeqs WGS400 done on a MGI DNB sequencer and mapped to the hg38 reference model (with 15x or greater read depth)  
* Sequencing.com WGS done in the BGI Labs (Nebula) and delivered mapped to a custom hs38s reference model
...
On the second tab (**Extract Data**), in the **Microarray Test** frame, hit the **Microarray Raw** button.  In the pop-up dialog window, leave the default recommended values and hit the **Generate** button.  After an hour, you will find four files in your **Output Directory**: a CombinedKit, a 23andMe\_v3, a 23andMe\_v5, and a 23andMe v3/v5 combo file.  The CombinedKit can be uploaded to GEDMatch and Geneanet.  Try the 23andMe v3/v5 combo for other sites.  But if not accepted, then the 23andMe\_v3 or 23andMe\_v5 can be uploaded to sites like MyHeritage, LivingDNA, and FamilyTreeDNA.  
...
A quick work-around to get a Build 38 Y and MT model is as follows. Load your just created **\_chrYM** subset Dante BAM into **WGS Extract**. Hit the **Index** button and then the **Realign** button.  After about 10 minutes, it will have created a **\_chrYM\_hs38** BAM file and have reloaded and indexed it for you automatically. You can ignore the low coverage warning (see the Stats to understand why it comes up). **Important**: You must now subset this new BAM (again) by clicking on the **Y and MT BAM** button (again). So now your **Output Directory** will have a BAM file, named like your original BAM, but with **\_chrYM\_hs38\_chrYM** appended to the name.  You will now use this Build 38 BAM file to upload to yFull.
...
Your available languages may vary depending on the source of your language file. Interface languages are easy to add.  See the [appendix on adding languages](#heading-1) at the end of this document.
...
**Note**: *You must select the (1) **Output Directory,** (2) a **BAM File** AND (3) click the **Stats** button BEFORE most other buttons become available. Hence why you see the grayed out buttons in the tabs of **figure 1.3** above.*  
...
There are a number of settings you can specify here.  Some of which can be left alone if you do not modify the installation.  There are two settings in this frame that must always be set before proceeding further.  The **Language** (which must be set before the main Window can even be shown) and the **Output Directory**.  
![][image7]  
...
The **Reference Library** is needed for the bulk storage of human reference genome assembly models and their related files.  20 \- 30 GB is usually sufficient and it can be more read-only in nature;  like the original **BAM file** folder(s).  The genomes and associated files are in the genome sub-folder of the reference library
...
After selecting a BAM file, the tool will immediately perform “quick” prep-work. For example, if the BAM index file already exists, the detailed stats will be gathered as if the **Stats** button was clicked.  Some of the stats will then be shown in this frame. Otherwise, without a BAM Index file or if you specify a CRAM file, then you will have to click the **Stats** button to run samtools idxstats. The detailed stats are needed before we can enable other buttons here and in other tabs. An example of the frame after selecting an indexed BAM is shown in Figure 1.7 above.  
...
#### 1.3.2.6 BAM Realign {#1.3.2.6-bam-realign}
...
| Build Class  | Build 37 | Build 38 |
| ----- | :---: | :---: |
| **UCSC / HG** | hg19 \* / hg37 | hg38 |
| **1K Genome** | hs37d5
human\_g1k\_v37 | hs38    (aka GCA\_\*GRCh38\_no\_\*set)
hs38DH \* (aka GRCh38\_full\_analysis\_set\_...) |
| **EBI / GRCh** | Homo\_sapiens.GRCh37 | Homo\_sapiens.GRCh38 |
...
There are no options for this button.  Once clicked, the program immediately starts the long process of realignment.  Often ½ a day to 6 or more days.  More details are given in the [special chapter on Unalign, Align and Realign a BAM](#5-unalign,-align-and-realign).
...
### 1.5.1 Haplogroups {#1.5.1-haplogroups}
...
* [*https://yfull.com/search-snp-in-tree/*](https://yfull.com/search-snp-in-tree/)  
...
We go into much more detail about these result windows in a [dedicated chapter on Haplogroups](#heading-2) later in the document.
...
WIth version 4, we have moved a normally hidden DEBUG mode frame from the Analysis tab to its own, separate DEBUG tab.  This tab will only be visible when DEBUG\_Mode is turned on.  All further description of DEBUG\_mode and the content of this Tab is in the [appendix on the DEBUG\_Mode](#12-appendix:-debug-/-developer-mode).  Only turn this mode on if requested by a developer or you have a special interest in one of its offered buttons. We only mention it here for completeness.
...
**Note**: *We have “ignored” the EBV / NC\_00705 alternate contiguous sequence in the “Others” section of the stats. Oral samples do not have this contamination and thus have very low mapping (like for Y in a biological female) and thus it throws off the stats.*
...
The v3 **Breadth of Coverage** was based on the **`samtools coverage`** command and used the default, non-zero mode.  That is what is still reported in the by-sequence summary section. It determines what percentage of the reference model has at least one read. It also calculates the actual mean of the read depth.  That latter value is not displayed but available in the saved **`.csv`** file generated when the button is clicked. The v4 **Breadth of Coverage** (for WGS and WES) is based on the **`samtools depth`** command (as was the v3 WES coverage calculation).  But now calculating more bin buckets instead of simple zero and non-zero buckets.
...
v4 saw the introduction of the Read Insert Size calculation. And a standard deviation for this value and the average read depth.  Look for more on the Insert Size in the [later special section](#2.8-read-insert-size).
...
Many may look at their BAM file results using a Genome Viewer such as IGV from the Broad Institute or GenomeBrowser from UCSC.  What is key here is they depict the aligned read segments as stacked like pancakes.  The base pairs of a chromosome are shown left to right (in order) per the reference mode;.  Each aligned read segment is stacked one on the other like pancakes.  So all the segment read values for a given base-pair are aligned vertically.  ![][image27]  
...
## 2.8 Read Insert Size {#2.8-read-insert-size}
...
**Note:** *National Geographic Genographic NextGen (2.0) Plus output is not yet available.  It includes a full Autosomal panel as well as 13K+ Y SNPs. FTDNA accepts the Y SNPs from National Geographic only if provided as an internal, company-to-company transfer and not via an autosomal import file. FTDNA historically only accepted Y SNPs from the National Geographic Genographic test and only as an internal transfer between organizations. As the NatGeoGeno public site is now defunct (Jun 2020), there is no longer a Y import path available.*  
...
| ISOGG Tree ([https://isogg.org/tree/](https://isogg.org/tree/)) | Search ISOGG for SNP ([https://isogg.org/tree/ISOGG\_YDNA\_SNP\_Index.html](https://isogg.org/tree/ISOGG_YDNA_SNP_Index.html)) |
| :---- | :---- |
| yFull ([https://yFull.com/tree](https://yfull.com/tree)) | Search yFull for SNP ([https://yfull.com/search-snp-in-tree/](https://yfull.com/search-snp-in-tree/)) |
| FamilyTreeDNA ([https://familytreedna.com/public/y-dna-haplotree/](https://familytreedna.com/public/y-dna-haplotree/))
[BigY Blocktree](https://www.familytreedna.com/my/bigy-blocktree) (requires their BigY test to see their BigY Block tree) | Search FTDNA for SNP ([https://familytreedna.com/public/y-dna-haplotree/](https://familytreedna.com/public/y-dna-haplotree/)) Note: Change from "View by Country" to "View by Variants" once on the page and then do a browser search for the SNP name |
| yTree BigTree 
([https://ytree.net/](https://ytree.net/))
(only R1b-P312 and below) | Search yTree for SNP ([https://www.ytree.net/SNPIndex.php](https://www.ytree.net/SNPIndex.php))  |
| Haplogroup-R ([http://haplogroup-r.org/tree/R.html](https://haplogroup-r.org/tree/R.html))
(currently, only R and below but expected to expand to a full tree) | Search Rtree for SNP ([https://haplogroup-r.org/variants.html](https://haplogroup-r.org/variants.html))  |
...
* Wikipedia ([https://en.wikipedia.org/wiki/Human\_Y-chromosome\_DNA\_haplogroup](https://en.wikipedia.org/wiki/Human_Y-chromosome_DNA_haplogroup))  
* Eupedia ([https://www.eupedia.com/genetics/phylogenetic\_trees\_Y-DNA\_haplogroups.shtml](https://www.eupedia.com/genetics/phylogenetic_trees_Y-DNA_haplogroups.shtml))  
* H600 Project ([https://h600.org/wiki/Haplogroup](https://h600.org/wiki/Haplogroup))  
* ISOGG ([https://isogg.org/wiki/Portal:Y-chromosome\_DNA](https://isogg.org/wiki/Portal:Y-chromosome_DNA)) \*   
...
| :---- | :---- | :---- | :---- | :---- | :---- |
| [rCRS](https://www.ncbi.nlm.nih.gov/nuccore/251831106) / CRS | 1999 / 1981 | 16,569 | H2a2a1 | GRCh38, HG38, (hs37d5) | 3107N in rCRS |
| [Yoruba](https://www.ncbi.nlm.nih.gov/nuccore/17981852) |  | 16,571 | L3e2b1a1
...
# 5 Unalign, Align and Realign {#5-unalign,-align-and-realign}
...
* **Unalign BAM (that is, BAM to FASTQ’**s  (1-2 hours)  
  (add 45 minutes if CRAM; final disk space 1x BAM size in Output Directory; intermediate about 200 GB in Temporary Files directory during the collate sort function)  
* **Create Reference Model Alignment Indices**  (2-3 hours)  
  (5 GB additional disk space in Reference Library; until you delete it)  
...
(2) The Reference Model selection pop-up is the same given when a BAM is loaded and the reference model cannot be identified.  With the caveat that all reference models are available for selection.  Whereas, in the BAM pop-up, the list of available buttons to select is limited to what content in the BAM indicates the reference model could possibly be. Once you select the desired reference model, click done.

The most common and suggested reference models are in the left column of the two columns shown. 1K Genome project models are suggested and what Dante Labs and Nebula Genomics use.  UCSC HG models are the next possible choice and what is used by ySeq and FamilyTreeDNA.  The last row are the two EBI GRCh reference models. See the [special section on reference model selection](#5.4-selecting-a-bam-reference-model) below for more information.
...
## 5.4 Selecting a BAM Reference Model {#5.4-selecting-a-bam-reference-model}
...
# 6 Reference Genomes {#6-reference-genomes}
 ...
 **`1) Exit                           20) hg38+hg002y_v2 (by ySeq) 3x`**  
 **`2) Recommended (@US NIH)          21) hg19 (yoruba; early Dante)`**  
 **`3) Recommended (@EU EBI)          22) hs38a (1K Gen)`**  
 **`4) T2T_v2.0 (PGP/HPP chrN) (Rec)  23) hs37 (1K Gen)`**  
 **`5) hs37d5 (Dante) (NIH) (Rec)     24) hs38d1v (Verily; unique)`**  
 **`6) hs38 (Nebula) (NIH) (Rec)      25) hg37 (rCRS; WGSE v1; BAD)`**  
 **`7) hs38d1 (Nebula new) (NIH)      26) hg01243v3 (PuertoRican1)`**  
 **`8) hs38DH (aDNA) (NIH) 3x         27) hg002xy_v2.7 (T2T)`**  
 **`9) hs38d1a (1K Gen+) (NIH)        28) hg002xy_v2 (T2T)`**  
**`10) human_g1k_v37 (NIH)            29) chm13y_v1.1 (HPP)`**  
**`11) hs37d5 (Dante) (EBI) (Rec)     30) chm13y_v1 (HPP)`**  
**`12) hs38 (Nebula) (EBI) (Rec)      31) T2T_v1.1 Draft`**  
**`13) hs38d1 (Nebula New) (EBI)      32) T2T_v1.0 Draft`**  
...
**`17) hs38d1s (by Sequencing)        36) GRCh37 (Ensembl) (patched)`**  
**`18) hg38 (ySeq)                    37) GRCh38- (Gencode) (primary)`**  
...
The script keeps querying for another option until the (1) **Exit** option is chosen. The **Recommended** options are the same as selecting the individual ones (items 4-6, 11-12).
...
Each human reference genome is around 1 GB in size and takes from a minute to an hour to download, index and process.  When doing an alignment, an additional 5 GB of index files are created alongside the reference genome. 
...
This expansion of information on the reference genome types is to help explain the [BAM Reference Model](#5.4-selecting-a-bam-reference-model) and the [BAM Realign button](#1.3.2.6-bam-realign) sections more fully. More information on downloadable reference genomes is in an external [Determining Your BAM Reference Model](https://bit.ly/34CO0vj) document.
...
| ----- | :---: | :---: |
| **UCSC / HG** | hg19 \*,  **hg37** | **hg38** |
| **1K Genome** | **hs37d5,**  human\_v37\_g1k, hs37 | **hs38**,
...
The 1000 Genomes project (**1K**) is the most common and suggested model class to always use.  **HG** (for historic, UCSC released Human Genome project models) and **EBI** (for the EBI Ensembl database) are two other common ones. **NCBI** is a fourth class and represents the original reference model with no changes for analysis. They are generally never encountered and not covered here.

**EBI** models are sometimes mistakenly referred to as the newer Genome Reference Consortium \- human (or **GRCh**) models. They traditionally are known for their numeric sequence naming and minimal changes from the reference genomes. But this only applies to the **Ensembl** database models on the **EBI** servers.  These are not to be confused with the **EBI** distributed Gencode consortium models that use the **HG** sequence naming and analysis set conventions found in the 1K Genome models. Gencode is a new consortium that includes UCSC, NCBI and others.  
...
The 1000 Genome project source models originated with the short model names starting with “**hs**”; but then go by many different names in practice.  For example, **hs37d5** is a superset of **hs37** which is a superset of **human\_g1k\_v37**. For convenience and a shorter designation, we name the latter as **hs37-** in the tool. This last one is derived from the original **NCBI37** reference model.

All 1K Genome models are generated by the bwa.kit program that came out of the 1000 Genome project and its BWA aligner tool.  The Build 38 models from the 1K Genome project are **hs38**, **hs38a**, and **hs38DH**. **GCA\_000001405.15\_GRCh38\_no\_alt\_analysis\_set** is the name often seen for **hs38** as it was captured and named that way in the NCBI archive server.  Often, NCBI is the only online source of these models. **Hs38a** also goes by **GCA\_000001405.15\_GRCh38\_full\_analysis\_set** for similar reasons.  And **hs38DH** is the original name for the commonly seen name **GRCh38\_full\_analysis\_set\_plus\_decoy\_hla**.  We recommend these 1K Genome sourced models be used as much as possible and encourage the shorter, easier to recognize names for them as well. There is also an additional **hs38d1** and **hs38d1a** model that did not come out of the 1K Genome project but were derived later from them.  We have given them this shorter name for convenience here. They tend to only be known by a longer **GCA\_000001405.15\_GRCh38\_xxxxx** name like the others.
...
SN’s | Notes |
| ----- | :---: | :---: | ----- |
| hs37d5.fa.gz | 1k37**g** | 86 | 1K Genome project model for Build 37 (with EBV) |
| hs38.fa.gz | 1k38**h**  | 195  | 1K Genome project model for Build 38 (aka GCA\_000001405.15\_GRCh38\_no\_alt\_analysis\_set) |
| chm13\_v2.0.fna.gz  | T2Tv20h | 25 | A unique, experimental model from the Telomere to Telomere consortium (New) |
...
For reasons on why to use one model over another, see [Heng Li’s blog post](https://lh3.github.io/2017/11/13/which-human-reference-genome-to-use) on the subject.  hg19\_WGSE suffers from issue 7 listed there (that no others do) and thus the main reason it should not be used.  hg19\_WGSE is not available from any other source.
...
# 7 Installing and Starting WGS Extract {#7-installing-and-starting-wgs-extract}
...
* [Windows 10 / 11 Desktop](#7.4-windows-10-/-11)  
* [Apple MacOS X / 11 / 12 / 13](#7.5-apple-macos-x-/-11--15)  
* [Ubuntu Linux LTS 18 / 20 / 22](#7.6-ubuntu-linux-lts-22,-20-\(18\))
...
* \_**`ubuntu.sh`** (Linux LTS 18, 20 and 22 on x86\_64), and   
* **`_windows.bat`** (Microsoft Windows 10 and 11 on x86\_64).  
...
## 7.4 Windows 10 / 11 {#7.4-windows-10-/-11}
...
If you have a previous **WGS Extract** release, copy the file contents of this new **WGSExtractv4/** installer folder into your previous release folder. So the new **Install\*** and other script files should be among your previous files.  Once there, you can then simply double-click on the **Install\_windows.bat** script file.  (Or open a **`cmd.exe`** or **`Powershell`** and run the **Install\_windows.bat** file directly via **`call Install_windows.bat`**)  (Upgrade functions are in the install scripts in v4.)
...
## 7.5 Apple MacOS X / 11 \-15 {#7.5-apple-macos-x-/-11--15}
...
For many reasons, we chose to have a terminal log of commands behind the application and installer.  But, much to our dismay, MacOS does not allow an application to control some of the settings for Terminal windows.  As a result, only on MacOS, a Terminal window log will be left on your screen (as shown below) after every run of the **Install, Update** or **Start** scripts.  ![][image50]
...
## 7.6 Ubuntu Linux LTS 22, 20 (18) {#7.6-ubuntu-linux-lts-22,-20-(18)}
...
See the [section below on permission changes](#7.6.3-possible-permission-changes-required) that may be necessary in your installation for Shell script files. The default on a new desktop is to open Shell scripts in the editor.  
...
### 7.6.3 Possible Permission Changes Required {#7.6.3-possible-permission-changes-required}
...
Workaround:None. Do not use the CombinedKit files as an import to those company sites. This is a limitation they have. Try the new 23andMe v3 & v5 file.  If not, just one of the individual, recommended files.
...
Workaround: Use **`DNA Kit Studio`** to compare microarray files generated with one you may have from the test lab directly for the same tester. But only on MS WIndows.  Use [David Pike’s online tools](https://www.math.mun.ca/~dapike/FF23utils/) otherwise.
...
Workaround: On Windows, enable Windows Subsystem for Linux (WSL 2\) and install BWA there (sudo apt-get install bwa).  Then turn on the [WSL BWA patch option](#10-appendix:-windows-wsl2-bwa-patch-option) as described in an appendix.
...
Workaround: You must grant **Screen Recording** permission to Terminal and Python Launcher to enable the Save function. This is shown in the screen capture below.  
![][image56]
...
Workaround: None. Must wait for VM support to improve. In developers mode now. WSLG is only available in Windows 11 and the default when WSL is installed there.
...
Workaround: Move the window to capture onto the primary screen before hitting save.
...
Workaround: The header is also saved into a text file in the Output Directory.  Maybe view the text version there with your favorite text file editor / viewer.
...
Workaround: Fork mimimap2 from github, compile and install yourself. Otherwise, you cannot align Nanopore long read FASTQ files under the MacOS with **WGS Extract**.
...
Workaround: None. Do not use these reference models as the target of an alignment. (Appears [HiSat2](http://daehwankimlab.github.io/hisat2/download/) may be designed to work with these models. But it is not available with any package provider that we have found so far \-- not Macports nor Homebrew on Mac, etc.)
...
Workaround: **WGS Extract** supports multiple invocations by the same user on the same machine; separating out the **Temporary Files** directory area on a per processorID basis.  So start a second copy of the program to continue doing other tasks during a long run (that does not completely consume the CPU, Memory and/or disk bandwidth).
...
Workaround: None. Try to avoid the platform until it becomes more stable. We are filtering the xcode warnings during the install.

## 8.2 Licenses and Redistributed Tools {#8.2-licenses-and-redistributed-tools}

See the folder **`open_source_libraries`** in the **WGS Extract** release home location.

| Tool | Author / Institution | Where Used | Notes |
| :---- | :---- | :---- | :---- |
| [Samtools, BCFTools, htsLib](http://www.htslib.org/) | Sanger Institute (Cambridge, et al) (original)
...
| [BWA MEM](https://github.com/lh3/bwa), [BWAMEM2](https://github.com/bwa-mem2/bwa-mem2)  |  | Re-alignment | Installed with package managers; Custom CygWin port. |
| [Extract23](https://github.com/tkrahn/extract23) v0.0
...
| [Haplogrep2](https://haplogrep.i-med.ac.at/) .xx
(distributed with WGSExtract) | Medical University of Innsbruck;  Institute of Genetic Epidemiology | Mitochondrial  haplogroup determination | JAR file redistributable |
| [yLeaf](https://cluster15.erasmusmc.nl/fmb/Yleaf_v2/index.html?lang=en) v2.x
...
| [Picard](https://broadinstitute.github.io/picard/), [GATK3 exec](https://console.cloud.google.com/storage/browser/gatk-software/package-archive/gatk;tab=objects?prefix=&forceOnObjectsSortingFiltering=false) / [docs](https://github.com/broadinstitute/gatk-docs), [Picard](https://broadinstitute.github.io/picard/), [GATK4](https://gatk.broadinstitute.org/hc/en-us), [IGV](http://software.broadinstitute.org/software/igv/)
...
[Python License](https://docs.python.org/3/license.html?highlight=icense)
[CygWin64 License](https://cygwin.com/licensing.html) | Everywhere (Win10 compiled versions of Unix utilities and libraries to support Win10 compiled bioinformatic tools) | **Win10 release only** (for Cygwin64) |
...
| [WGS Extract Manual](http://bit.ly/36Jdpnq) | Randy Harr | Documentation | Available with the tool under the same license arrangement |

These are not incorporated into the tool but are more general inspirations or companion tools.

| [BAM Analysis Kit](http://www.y-str.org/2014/04/bam-analysis-kit.html) and others (note used by inspired by) | Felix Chandrashakar / Immanuel, y-str.org
(or ([re-port by Teepean](https://github.com/teepean/BAM-Analysis-Kit)) | Initial idea for Haplogroup caller inclusion; and general tool flow effort | Not directly incorporated; but many tool flows and features inspired by this very early work |
| :---- | :---- | :---- | :---- |
| [DNA Kit Studio](http://dnagenics.com/dna-kit-studio/)
...
| **bwa [^5]** | 0.7.17-r1198-dirty | 0.7.17-r1198-dirty |  |  |  |  |  | 0.7.17-r1198-dirty |
...
| **Python3** | 3.8.9 | 3.10.2 | 3.8.3 | 3.9.12 [^6] | 3.6.9 | 3.8.5 | 3.10.4 | 3.10.4 |
...
## 8.3 Release Notes  {#8.3-release-notes}
...
* Add option to annotate BAM header SN’s with M5 tag (like exists in CRAM) to more accurately document the Ref Model used.  See [https://genome.sph.umich.edu/wiki/BamUtil:\_polishBam\#Add\_MD5\_and\_UR\_tags\_to\_SQ\_Headers\_.28--fasta.29](https://genome.sph.umich.edu/wiki/BamUtil:_polishBam#Add_MD5_and_UR_tags_to_SQ_Headers_.28--fasta.29) for utility (but pretty simple to do in python and using samtools reheader command).  Values are already in reference model DICT file.  
...
* ~~Add BAM to paired-end FASTQ button (unalign): (added but not callable by user)~~  
  **~~`samtools collate -o collated.bam orig.bam`**  `# change to name sorted in original`~~  
  **~~`samtools fastq -1 bam_R1.fastq.gz -2 bam_R2.fastq.gz -0 /dev/null -s /dev/null -n collated.bam`**  `# compresses if .gz ext; 2nd line`~~  
  **~~`rm collated.bam`**~~  
  ~~(check [Kyle Day’s One liner](https://www.facebook.com/notes/dante-labs-and-nebula-genomics-customers/extracting-r1-and-r2-reads-from-your-bam-for-realignment/488019108459920/)?)  ([https://bioinformatics.stackexchange.com/questions/8938/convert-bam-to-properly-paired-fastq-files](https://bioinformatics.stackexchange.com/questions/8938/convert-bam-to-properly-paired-fastq-files))  ([Correct, too complicated version](https://gist.github.com/darencard/72ddd9e6c08aaff5ff64ca512a04a6dd) that uses bam2fastq from bedtools?)~~  
...
* ~~Use import multiprocessing; multiprocessing.cpu\_count() to set number of parallel threads in htslib commands (maybe count \-1 to leave one CPU for interactive work)~~   
...
* ~~Cleanup\_temp\_dir processing: handle yleaf dir tree left after crash~~  
...
* SNP Variant Caller (BAM to VCF) (bcftools call, GATK) (RAW, filtered)  
  * ~~mpileup parallel calls per chromosome for speedup ([see TKrahn](https://gist.github.com/tkrahn/ef62cfaab678f447ea53ddee09ce0eb2)) (easy once sub-process handling added to allow non-blocking command startup)~~  
    ~~(added but then removed to Debug\_mode section as:~~ does not work; at least not on 1KGenome reference genomes where bcftools know about alt contigs. Would need to merge and then do calls knowing which alt contigs to keep with which chromosomes.)  
...
* VCF file annotator (add rsID, SNP Name, genes, etc; without merge); Annovar, Nirvana  
  ...
  [https://www.biostars.org/p/393009/](https://www.biostars.org/p/393009/), [https://www.biostars.org/p/297578/](https://www.biostars.org/p/297578/), [https://software.broadinstitute.org/software/igv/viewing\_vcf\_files](https://software.broadinstitute.org/software/igv/viewing_vcf_files), [https://bioinformatics.stackexchange.com/questions/11497/vcf-file-to-gene-annotations-and-or-genes](https://bioinformatics.stackexchange.com/questions/11497/vcf-file-to-gene-annotations-and-or-genes), [https://uswest.ensembl.org/info/docs/tools/vep/index.html](https://uswest.ensembl.org/info/docs/tools/vep/index.html), [https://uswest.ensembl.org/info/docs/tools/vep/script/index.html](https://uswest.ensembl.org/info/docs/tools/vep/script/index.html),   
...
* Add a genome browser:  
  * Broad Institute [IGV](https://software.broadinstitute.org/software/igv/) to view BAM and similar files;  
    ...
    * Create [.genome](https://software.broadinstitute.org/software/igv/LoadGenome) that has reference genome and GFF annotations (json)  
...
* Rewrite reference/genomes/process\_reference\_genomes.sh and get\_reference\_genomes. sh into python and add to the reference library processor.   
...
* Utilize samtools v1.11 change to remove many single character options and have global options such as “--reference reference=fasta\_file” and “--write\_index”. And to also use the OUTFMT options to specify the temp directory for intermediate files.on all commands  
* Expand Python reference\_library code to automatically recognize missing reference genome files and download on demand (as needed).  
...
* Make an action\_button “register” routine with parameters: like which BAM characteristics it relies on (chrom\_types, sorted and index BAM file, etc). Then make checking of needed state before running the action button more automatic; is now hand-coded in a few places.  
...
* ~~Save settings and auto-reload if available (language, BAM file, output dir, temp\_dir, refgen library)~~  
* ~~Add cleanup in process\_bam\_header for Y and/or MT and/or unmapped only BAMs (summary column mostly; reference and sort model settings)~~  
...
* ~~Inline extract23\_script\_template.txt; simplify piped sed command; Use f-strings~~  
* ~~Adjusted Autosomal reference files to remove extraneous “combined-RECOMMENDED\_” in name~~  
...
* ~~Rewrite determine\_reference\_genome to simply get header and then do “grep” inside the memory copy of header; save header with BAM stats in global area.~~   
...
1.1.1a Stats Button (initial stats page)
...
1.1.1b Coverage Button on the Stats page

**`% samtools coverage -o your_coverage.csv your.bam`**

1.1.1c WES Average Depth Button on the Stats page 
...
1.1.6 Realign (and Align and Unalign)  
...
1.1.6a  Unalign (internal, part of Realign)  
...
1.1.6b  Align (internal, part of Realign)
...
# 10 Appendix: Windows WSL2 BWA Patch Option {#10-appendix:-windows-wsl2-bwa-patch-option}
...
This feature can also be toggled on and off when the program is in [DEBUG\_MODE (see appendix)](#12-appendix:-debug-/-developer-mode).
...
So edit the **`language.xlsx`** file.  Save in the same program/ directory.  And then simply start the **WGS Extract** program or hit the “reload” button when in DEBUG\_MODE to have the new **`language.xlsx`** file read in and used. It is that simple.  A user created the Portugues translation in an hour or so.
...
# 12 Appendix: DEBUG / Developer mode {#12-appendix:-debug-/-developer-mode}
...
**DEBUG\_MODE**: When DEBUG\_MODE is on, there is now a fourth tab added to the main window that is called DEBUG.  Here are the current content but it is due to change often..  
![][image60]  
...
[^1]:  Taken from “[Coverage Depth Recommendations](https://www.illumina.com/science/technology/next-generation-sequencing/plan-experiments/coverage.html)” from Illumina’s website.
...
[^4]:  See [http://www.beholdgenealogy.com/blog/?p=2879](http://www.beholdgenealogy.com/blog/?p=2879) for one anecdotal example
