#!/usr/bin/env bash
# WGS Extract Reference Library start script (generic Linux)
# Copyright (C) 2022-23 Randolph Harr
# Copyright (C) 2022-23 Aaron Balagan
#
...
source scripts/zxterm_linux.sh                  || { echo "ERROR: Cannot source scripts/zxterm_ubuntu.sh" ; exit 1 ; }

export linux_type="micromamba"                  # Only place we set before zcommon.sh; to indicate micromamba linux

declare microdir micromambax bashx wgse_FP
source scripts/zcommon.sh "$wgse_FP"
