...
*** do any regression testing on the WGS Extract program itself; loop
    until satisfied. ***
...
There are six versioned packages (four for everyone; two for just windows):
  ...
  (unversioned)                    -            _builder.zip    (Windows only) Cygwin64 base release with compilation
                                                                    environment to make _bioinfo release. Not
                                                                    distributed.

  release.json                     release      (in installer)  Release track and base URLs to latest version jsons
  latest-release-Dev.json          release      -               One for each track; generated in make_release and
                                                                uploaded when appropriate to Github.

  make_release.sh / .txt           program (but deleted)        Used to create the packages and JSONs.

  cygwin64/make_cygwin64.sh         cygwin64                    Stores the URL and version for the new cygwin64
                                                                    release and creates the cygwin64.json file. This
                                                                    because there is no development directory to store
                                                                    the JSON file in.
  cygwin64/usr/local/make_bioinfo.sh bioinfo                    Stores the URL and version for the new bioinfo tools
                                                                    release on Windows; creates bioinfo.json file.
